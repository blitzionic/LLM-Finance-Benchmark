...
from gpt_eval import ask_gpt_direct, ask_gpt_rag
...
Please choose the correct answer: A, B, C, or D."""
        ...
        dataset.loc[index, "guess"] = answer.strip()  # Save only the letter
...
def benchmark_with_rag(dataset, client):
    pbar = tqdm(dataset.iterrows(), total=len(dataset), desc="Benchmarking RAG Answers")
    for index, row in pbar:
        question = f"""Question:
{row['question']}
A. {row['A']}
...
Please choose the correct answer: A, B, C, or D."""
        ...
        # Include the hint from the dataset
        hint = row['hint']
        
        # Query the RAG method
        answer = ask_gpt_rag(question, hint, client, model="gpt-4o")
        dataset.loc[index, "guess"] = answer.strip()  
...
def main():
    ...
    parser.add_argument("-m", "--method", choices=["direct", "cot", 'rag'], default="direct", help="Answering method")
    ...
    if args.method == "cot":
        ...
        print("Using RAG...")
        results = benchmark_with_rag(dataset, client)
    else:
